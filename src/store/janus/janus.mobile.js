...
Janus.init = function (options) {
  ...
  if (Janus.initDone === true) {
    ...
    Janus.ajax = function (params) {
      ...
      if (params.requesting !== "long poll") Janus.log(params);
...
export default function Janus(gatewayCallbacks) {
  ...
  gatewayCallbacks.ondisconnect = typeof gatewayCallbacks.ondisconnect == "function" ? gatewayCallbacks.ondisconnect : Janus.noop;
  ...
  this.destroyHandler = function(callbacks){
    destroyHandle(callbacks);
  }
  ...
  function eventHandler() {
    ...
    if (!connected) {
      Janus.warn("Is the gateway down? (connected=false) from eventHandler");
      this.destroy && this.destroy();
      gatewayCallbacks.ondisconnect && gatewayCallbacks.ondisconnect(true);
      // store.dispatch({ type: "audio/reInit", paylaod: {} });
      return;
    }
    ...
    Janus.ajax({
      ...
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        // this.destroy();
        // store.dispatch({ type: "audio/reInitialize", paylaod: {} });
  ...
  function destroySession(callbacks, syncRequest) {
    ...
    if (!connected) {
      Janus.warn("Is the gateway down? (connected=false) from destroySession");
      callbacks.success();
      return;
    }
  ...
  function createHandle(callbacks) {
    ...
    if (!connected) {
      Janus.warn("Is the gateway down? (connected=false) from createHandle");
      callbacks.error("Is the gateway down? (connected=false) from createHandle");
      return;
    }
  ...
  function sendMessage(handleId, callbacks) {
    ...
    callbacks.error = typeof callbacks.error == "function" ? callbacks.error : Janus.noop;
    if (!connected) {
      Janus.warn("Is the gateway down? (connected=false) from sendMessage");
      callbacks.error("Is the gateway down? (connected=false) from sendMessage");
  ...
  function sendTrickleCandidate(handleId, candidate) {
    if (!connected) {
      Janus.warn("Is the gateway down? (connected=false) from sendTrickleCandidate");
      return;
    }
    var request = {
      janus: "trickle",
  ...
  function destroyHandle(handleId, callbacks, syncRequest) {
    ...
    if (!connected) {
      Janus.warn("Is the gateway down? (connected=false) destroyHandle");
      callbacks.error("Is the gateway down? (connected=false) destroyHandle");
      return;
    }
  ...
  function streamsDone(handleId, jsep, media, callbacks, stream) {
    ...
    Janus.debug(pc_constraints, pc_config);
  ...
  function prepareWebrtc(handleId, callbacks) {
    console.log("preparing webrtc");
    callbacks = callbacks || {};
    callbacks.success = typeof callbacks.success == "function" ? callbacks.success : Janus.noop;
    callbacks.error = typeof callbacks.error == "function" ? callbacks.error : webrtcError;
    var jsep = callbacks.jsep;
    var media = true; //callbacks.media;
    var pluginHandle = pluginHandles[handleId];
    if (pluginHandle === null || pluginHandle === undefined || pluginHandle.webrtcStuff === null || pluginHandle.webrtcStuff === undefined) {
      Janus.warn("Invalid handle");
      callbacks.error("Invalid handle");
    ...
    if (isAudioSendEnabled(media) || isVideoSendEnabled(media)) {
      ...
      if (videoSupport === true && media != undefined && media != null) {
        ...
        if (media.video && media.video != "screen" && media.video != "window") {
          ...
          console.log("screen or window");
          if (window.location.protocol !== "https:") {
            // Screen sharing mandates HTTPS
            console.log("window protocol not https");
          ...
          window.addEventListener("message", function (event) {
            console.log("window event message");
  ...
  function createOffer(handleId, media, callbacks) {
    ...
    mediaConstraints = {
      mandatory: {
        ...
        OfferToReceiveVideo: isVideoRecvEnabled(false),
      },
    };
    // }
  ...
  function createAnswer(handleId, media, callbacks) {
    ...
    config.pc.createAnswer(
      function (answer) {
        Janus.debug(answer);
        if (config.mySdp === null || config.mySdp === undefined) {
          Janus.log("Setting local description create answer");
              ...
              if (res.type == "ssrc" && res.stat("googFrameHeightReceived")) {
                ...
                if (config.bitrate.bsbefore === null || config.bitrate.tsbefore === null) {
                  ...
                  var bitRate = Math.round(
                    ((config.bitrate.bsnow - config.bitrate.bsbefore) * 8) / (config.bitrate.tsnow - config.bitrate.tsbefore)
                  );
